...
export class BarSliderChartModel {
  ...
  protected chartName: string;

  private constraints?: BarSliderConstraints;

  private axis: BarSlidertAxis;

  private segments: BarSliderSegment[];
  ...
  AxisTicks(): BarSliderTickPosition[] {
  ...
  ChartRegions(): BarSliderRegionPosition[] {
    return Object.values(this.axis.ticks)
      .slice(1)
      .map((tick) => ({
        value: this.calculateRawPosition(tick.value),
        styleClass: this.segmentstylePrefix(tick.value),
      }));
  }

  private segmentstylePrefix(value: number): string {
    const segment = this.getSegmentByTick(value);
    return segment?.stylePrefix
      ? `${this.chartName}-${segment.stylePrefix}`
      : '';
  }
