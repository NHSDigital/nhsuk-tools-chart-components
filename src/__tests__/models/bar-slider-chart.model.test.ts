...
const mockConfig: BarSliderConfig = {
  ...
  segments: [
    ...
    {
      stylePrefix: 'mock-style-4',
      weight: 30,
    },
    {
      stylePrefix: 'mock-style-5',
      weight: 10,
    },
  ],
};
  ...
  it('expect the chart segments to contain the css style for the segment', () => {
    ...
    model.ChartRegions().forEach((segment, id) => {
      expect(segment.styleClass).toBe(
        `${mockConfig.name}-${mockConfig.segments[id].stylePrefix}`
      );
    });
  });
  ...
  it('when invalid ranges are set the css style defaults to empty string', () => {
    const config = {
      ...
      segments: [
        ...
        { start: 10, end: 40, stylePrefix: 'mock-style-2', weight: 40 },
      ],
    };
    ...
    model.ChartRegions().forEach((segment, id) => {
      ...
      if (
        ...
        expect(segment.styleClass).toBe(
          `${config.name}-${config.segments[id].stylePrefix}`
        );
      }
    });
  });
